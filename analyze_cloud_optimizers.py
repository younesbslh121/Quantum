...
def prepare_for_hardware(qubit_op, backend):
    ...
    print(f"   [SUCCESS] Circuit transpilé pour {backend.name}")
...
def run_vqe_with_optimizer(optimizer, opt_name, backend, isa_ansatz, isa_observable, num_params):
    ...
    def cost_function(params):
        ...
        evs_data = job.result()[0].data.evs
    ...
    print(f"   [SUCCESS] {opt_name} terminé en {elapsed:.2f}s | Énergie = {result.fun:.8f} Ha")
    return iterations, energies, result, elapsed


def save_analysis(all_results, exact_energy, backend_name):
    print("\n[SAVE] Sauvegarde des résultats d'analyse")
...
def main():
    print("")
    print("    [SCIENCE] Comparaison COBYLA vs SPSA sur IBM Quantum       ")
    print("")
    ...
    print(f"\n[WARNING] Lancement des exécutions (Max itérations = {MAX_ITERATIONS})")
    ...
    print("\n[SUCCESS] Analyse terminée. Exécutez le script de tracé pour visualiser.")
